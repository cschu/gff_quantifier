...
# 		self.seqdb = {
# 			db_seq.seqid: db_seq.id
# 			for db_seq in self.dbsession.query(db.AnnotatedSequence).all()
# 		}

# 	def query_sequence(self, seqid):
		
# 		db_sequence = self.dbsession.query(db.AnnotatedSequence).filter(db.AnnotatedSequence.id == self.seqdb.get(seqid)).one_or_none()
# 		if db_sequence is None:
# 			return None
# 		categories = tuple()
# 		for cat_features in db_sequence.annotation_str.strip().split(";"):
