...
class PandaCoverageProfiler(PandaProfiler):
    ...
    def update_coverage(self, aln_hits):
        ...
        #             .update({p: 1 / n_aln for p in range(hit.cov_start, hit.cov_end + 1)})
    ...
    def _calc_coverage(self):
            ...
            # ):
            #     uniq_cov = self._coverage_data.get(False, {}).get(key, Counter())
            #     ambig_cov = self._coverage_data.get(True, {}).get(key, Counter())
            #     length = key[2] - key[1] + 1
            #     len_both = len(set(uniq_cov).union(ambig_cov))
