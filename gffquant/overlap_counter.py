...
class OverlapCounter(dict):
	...
	def update_coverage_intervals(self, overlap, overlap_extent=None, features=None, ambig_aln=False):
		if ambig_aln:
			self.ambig_coverage.setdefault(overlap.rname, list()).append(overlap)
		else:
			overlap_extent = overlap.calc_ref_overlap() if overlap_extent is None else overlap_extent
			for feature in overlap.features.split(","):
				self.coverage_intervals.setdefault(overlap.ref, dict()).setdefault((overlap.ref_start, overlap.ref_end, feature), Counter())[overlap_extent] += 1
	def summarise_coverage(self):
		...
		for ref, intervals in self.coverage_intervals.items():
			for (start, end, feature), overlaps in intervals.items():
				ref_coverage.setdefault((ref, start, end), Counter())
				pos_feature_dict.setdefault((ref, start, end), set()).add(feature)
				for (ovl_start, ovl_end), count in overlaps.items():
					dist1_ref_coverage[(ref, start, end)] += count
					for p in range(ovl_start, ovl_end):
						ref_coverage[(ref, start, end)][p] += count

		for (ref, start, end), coverage in ref_coverage.items():
			ref_coverage[(ref, start, end)] = sum(coverage) / (end - start + 1)

		ambig_ref_coverage = Counter()
		for rname, overlaps in self.ambig_coverage.items():
			where = Counter()
			for ovl in overlaps:
				pos_feature_dict.setdefault((ovl.ref, ovl.ref_start, ovl.ref_end), set()).update(ovl.features.split(","))
				where[(ovl.ref, ovl.ref_start, ovl.ref_end)] += 1
			...
			if n_uniq_reads:
				for pos in uniq_dist:
					ambig_ref_coverage[pos] += uniq_dist[pos] / n_uniq_reads # or scale with n_ambig?
			else:
		...
		for pos in set(ref_coverage).union(ambig_ref_coverage):
			for domtype in pos_feature_dict.get(pos, set()):
				domain_cov.setdefault(domtype, Counter())
