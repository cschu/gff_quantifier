...
class FeatureQuantifier:
	...
	def process_alignments(self, bam):
		...
		with open(self.out_prefix + ".ambig_tmp.txt", "wt") as ambig_out_tmp:
			for aln_count, aln in bam.get_alignments(allow_multiple=True, allow_unique=True, disallowed_flags=0x800):
				...
				elif aln.is_paired() and aln.rid == aln.rnext and not aln.flag & 0x8:
